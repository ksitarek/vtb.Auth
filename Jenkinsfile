...
pipeline {
    ...
    environment {
        SONARQUBE_PROJECT_KEY = "vtb-auth"
    ...
    stages {
        ...
        stage('Publish Permissions') {
            steps {
                sh 'dotnet publish vtb.Auth.Permissions/vtb.Auth.Permissions.csproj --no-build --no-restore -o ./out -c Release'
        ...
        stage('Publish JWT') {
            steps {
                sh 'dotnet publish vtb.Auth.Jwt/vtb.Auth.Jwt.csproj --no-build --no-restore -o ./out -c Release'
                archiveArtifacts artifacts: 'out/*', fingerprint: true
